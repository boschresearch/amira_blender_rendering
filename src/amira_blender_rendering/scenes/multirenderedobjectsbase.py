...
class MultiRenderedObjectsBase(ABC, abr_scenes.BaseSceneManager):
    ...
    def postprocess(self):
        ...
        results = []  # TODO: adapt and use ResultsCollection from aps??
        for fname_mask, obj in zip(self.compositor.fname_masks, self.objs):
            corners2d = self.compute_2dbbox(fname_mask)
            aabb, oobb, corners3d = self.compute_3dbbox(obj)
            r = self.build_render_result(obj, corners2d, corners3d, aabb, oobb)
            results.append(r)
        self.save_annotations(results)  # corners2d, corners3d, aabb, oobb)
    ...
    def setup_compositor(self):
        ...
        # possible.
        # Differently from RenderedObjectsBase, here self.objs is already a list of objects
    ...
    def build_render_result(self, obj, corners2d, corners3d, aabb, oobb):
        """Build render result of an object."""
        ...
        t = np.asarray(abr_geom.get_relative_translation(obj, self.cam))
        R = np.asarray(abr_geom.get_relative_rotation(obj, self.cam).to_matrix())
        render_result = PoseRenderResult(obj.name, None, None, None, None, None, None,
    ...
    def save_annotations(self, results):
        """Save annotations of a render results."""
        ...
        json_data = [r.state_dict() for r in results]  # list of state dicts to dump
